...
namespace Core.Meditor.Product.Commend.Handleing
{
        ...
        IRequestHandler<DeleteProductModelQueries, Response<string>>
    {
        ...
        {
                var ProductMapping = _mapper.Map<Domain.Models.Product>(request);

                if (ProductMapping == null) return BadRequest<string>("Product Invald");
                
                var result = await _productServices.AddProductAsync(ProductMapping, request.FormImages);
                if (result.Succesd) return Created("");
